	...
	protogen.Options{}.Run(func(gen *protogen.Plugin) error {
		...
		for _, f := range gen.Files {
			...
			if importPath == "" || pkgName == "" {
				importPath, pkgName = f.GoImportPath, f.GoPackageName
			}
			rpcs = append(rpcs, generateFile(gen, f)...)
		}
		generateMsgDbg(gen, importPath, pkgName, rpcs)
		return nil
	})
}

func generateMsgDbg(gen *protogen.Plugin, importPath protogen.GoImportPath, pkgName protogen.GoPackageName, rpcs []rpc) {
	g := gen.NewGeneratedFile("proto/msgdebug.go", importPath)
	...
	g.P(`    "hash/adler32"`)
	g.P()
	g.P(`    "github.com/protocolbuffers/protoscope"`)
	g.P(`    "google.golang.org/protobuf/encoding/prototext"`)
	g.P(`    "google.golang.org/protobuf/proto"`)
	g.P(")")
	g.P()
	g.P("func MsgDbg(data []byte, isReq bool) (string, error) {")
	g.P(`    if len(data) < 6 {`)
	g.P(`    	return "", fmt.Errorf("frame is too short")`)
	g.P(`    }`)
	g.P(`    checksum := binary.BigEndian.Uint32(data[len(data)-4:])`)
	g.P(`    if want := adler32.Checksum(data[:len(data)-4]); checksum != want {`)
	g.P(`    	return "", fmt.Errorf("checksum doesn't match: got=%08X, want=%08X", checksum, want)`)
	g.P(`    }`)
	g.P()
	g.P(`    tag := binary.BigEndian.Uint16(data[0:2])`)
	g.P()
	g.P("    hdrReady := false")
	g.P("    var hdrLen uint16")
	g.P("    var hdrPayload []byte")
	g.P("    var hdr *MessageHeader")
	g.P()
	for _, rpc := range rpcs {
		if rpc.msgTag.Req != nil {
			g.P("    if tag == ", rpc.msgTag.GetReq(), " {")
			if rpc.msgTag.Header != nil {
				g.P("        if !hdrReady {")
				g.P("            hdrLen = binary.BigEndian.Uint16(data[3:5])")
				g.P("            hdrPayload = data[5 : 5+hdrLen]")
				g.P("            hdr = &MessageHeader{}")
				g.P("            if err := proto.Unmarshal(hdrPayload, hdr); err != nil {")
				g.P(`                return "", fmt.Errorf("unmarshalling header: %v", err)`)
				g.P("            }")
				g.P("            hdrReady = true")
				g.P("        }")
				g.P("        if hdr.GetTag1() == ", rpc.msgTag.GetHeader().GetTag1(), " && hdr.GetTag2() == ", rpc.msgTag.GetHeader().GetTag2(), " {")
				g.P("            payload := data[5+hdrLen : len(data)-4]")
				g.P("            var msg proto.Message")
				g.P("            if isReq {")
				if !isEmpty(rpc.input) {
					g.P("                msg = &", rpc.input.GoIdent.GoName, "{}")
				}
				g.P("            } else {")
				if !isEmpty(rpc.output) {
					g.P("                msg = &", rpc.output.GoIdent.GoName, "{}")
				}
				g.P("            }")
				g.P("            if err := proto.Unmarshal(payload, msg); err != nil {")
				g.P(`                return "", fmt.Errorf("unmarshalling: %v\n\tframe: %04X%s", err, len(data), hex.EncodeToString(data))`)
				g.P("            }")
				g.P(`            return fmt.Sprintf("%s{ %s } %s{ %s }", hdr.ProtoReflect().Descriptor().Name(), prototext.Format(hdr), msg.ProtoReflect().Descriptor().Name(), prototext.Format(msg)), nil`)
				g.P("        }")
			} else {
				g.P("        payload := data[3 : len(data)-4]")
				g.P("        msg := &", rpc.input.GoIdent.GoName, "{}")
				g.P("        if err := proto.Unmarshal(payload, msg); err != nil {")
				g.P(`            return "", fmt.Errorf("unmarshalling: %v\n\tframe: %04X%s", err, len(data), hex.EncodeToString(data))`)
				g.P("        }")
				g.P(`        return fmt.Sprintf("%s{ %s }", msg.ProtoReflect().Descriptor().Name(), prototext.Format(msg)), nil`)
			}
			g.P("    }")
		}
		if rpc.msgTag.Resp != nil {
			g.P("    if tag == ", rpc.msgTag.GetResp(), " {")
			if rpc.msgTag.Header != nil {
				g.P("        if !hdrReady {")
				g.P("            hdrLen = binary.BigEndian.Uint16(data[3:5])")
				g.P("            hdrPayload = data[5 : 5+hdrLen]")
	...
	g.P()
	g.P("    psOpts := protoscope.WriterOptions{")
	g.P("    	 NoQuotedStrings:        false,")
	g.P("    	 AllFieldsAreMessages:   false,")
	g.P("    	 ExplicitWireTypes:      false,")
	g.P("    	 NoGroups:               true,")
	g.P("    	 ExplicitLengthPrefixes: false,")
	g.P("    }")
	g.P()
	g.P("    if hdrReady {")
	g.P("        payload := data[5+hdrLen : len(data)-4]")
	g.P(`        return fmt.Sprintf("%s{ %s } ???{ %s }", hdr.ProtoReflect().Descriptor().Name(), prototext.Format(hdr), protoscope.Write(payload, psOpts)), nil`)
	g.P("    } else {")
	g.P("        payload := data[3 : len(data)-4]")
	g.P(`        return fmt.Sprintf("[tag=%d] ???{ %s }", tag, protoscope.Write(payload, psOpts)), nil`)
	g.P("    }")
	g.P("}")
}

func generateFile(gen *protogen.Plugin, file *protogen.File) []rpc {
	var rpcs []rpc
	...
	for _, svc := range file.Services {
		rpcs = append(rpcs, generateServiceClient(g, svc)...)
	}
	return rpcs
}
...
func generateServiceClient(g *protogen.GeneratedFile, svc *protogen.Service) []rpc {
	// Collect RPC info
	var rpcs []rpc
